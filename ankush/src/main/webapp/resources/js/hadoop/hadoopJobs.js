...
com.impetus.ankush.hadoopJobs={
		...
		getJobTrackerStatus : function(page, clusterId) {
			var statusUrl = baseUrl + '/monitor/' + clusterId + '/jobtrackerstatus';
			$.ajax({
				'type' : 'GET',
				'url' : statusUrl,
		...
		addJobArgumentRow : function() {
			...
			var jobArgumentRow = [];
			var colArgument = '<input type="text" data-toggle="tooltip" title="Job Argument-'+(index_JobArguments+1)+'" data-placement="right" class="input-large" id="jobArgument-'+(index_JobArguments+1)+'" placeholder="Argument" />';
		...
		submitJobs : function(clusterId_JobSubmit) {
			...
			if(com.impetus.ankush.commonMonitoring.clusterTechnology == 'Hybrid')
				submitJobUrl = baseUrl + '/monitor/' + clusterId_JobSubmit + '/submitjob?technology=Hadoop';
			else
				submitJobUrl = baseUrl + '/monitor/' + clusterId_JobSubmit + '/submitjob';
		...
		getApplicationMonitoringPageContent : function(clusterId){
			clusterId_JobMonitor = clusterId;
			var jobMonitorUrl = baseUrl + "/monitor/" + clusterId_JobMonitor + "/hadoop2applist";
				...
				if(result.output.status == true){
					if(result.output.hadoop2applist == undefined){
						...
						for(var i = 0 ; i < result.output.hadoop2applist.length ; i++){
							var navigationTopology = "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;<a href='#' onclick='com.impetus.ankush.hadoopJobs.appMonitorDrillDownPage(\""+result.output.hadoop2applist[i].id+"\",\""+result.output.hadoop2applist[i].applicationType+"\");'><img src='"
							+ baseUrl
...
//							                            result.output.hadoop2applist[i].clusterId,
							                            result.output.hadoop2applist[i].startedTime,
//							                            result.output.hadoop2applist[i].finishedTime,
		...
		appMonitorDrillDownLoad : function(appid,applicationType){
			var clusterId_JobMonitor = com.impetus.ankush.commonMonitoring.clusterId;
			var appMonitorUrl = baseUrl + "/monitor/" + clusterId_JobMonitor + "/appdetails?appid="+appid;
				...
				if(result.output.status == true){
					if(result.output.appattempts == undefined){
						...
						for(var key  in  result.output.appdetails){
							...
							$("#all_tiles_appInfo").append('<div class="row-fluid"><div class="span2 text-right">'+text+'</div><div class="span2 text-left" style="color:#000000">'+result.output.appdetails[key]+'</div></div>')
						}
				}		
			});
		...
		appMonitorDrillDownLoad_jobDetail : function(appid,applicationType){
			var appMonitorUrl = baseUrl + "/monitor/" + clusterId_JobMonitor + "/appjobinfo?appid="+appid;
			com.impetus.ankush.placeAjaxCall(appMonitorUrl,'GET',true,null,function(result){
				if(result.output.status == true){
		...
		loadJobsMonitoring_Table : function(strJobState) {
			...
			for(var i = 0; i < jobCount_JSON; i++) {
				if(jobListData[i].jobState == currentJobState || currentJobState == 'ALL') {
					...
					var colJobId = '<span style="font-weight:bold;" id="jobId-'+jobCount_Table+'">'+jobListData[i].jobId+'</span>';
					var colJobName = '<span style="font-weight:bold;" id="jobName-'+jobCount_Table+'">'+jobListData[i].jobName+'</span>';
					...
					var colMapProgress = '<span style="font-weight:bold;" id="mapProgress-'+jobCount_Table+'">'+jobListData[i].mapProgress+'</span>';
					var colReduceProgress = '<span style="font-weight:bold;" id="reduceProgress-'+jobCount_Table+'">'+jobListData[i].reduceProgress+'</span>';
					var colJobState = '<span style="font-weight:bold;" id="jobState-'+jobCount_Table+'">'+jobListData[i].jobState+'</span>';
