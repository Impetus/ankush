...
com.impetus.ankush.configurationNew={
		...
		editRow : function(userDataTable, nRow){
		    ...
		    jqTds[5].innerHTML = '<div style=""><select id="endis-'+index+'" onclick="com.impetus.ankush.configurationNew.updateStatusField('+index+')"><option>Enabled</option><option>Disabled</option></select></div>',
		    jqTds[6].innerHTML = "<div><a class='deleteUser' id='deleteUser-"+index+"' href='#' onclick='com.impetus.ankush.configurationNew.deleteUser(this);'><input type='image' style='width:22px;height:22px;' src='"+baseUrl+"/public/images/newUI-Icons/circle-minus.png' ></input></a></div>";
		...
		loadUserTableSection : function(result){
			...
			if(Object.keys(result.output.users).length != 0){
				for(var i=0;i<result.output.users.length;i++){
					...
					console.log(users);
					var deleteUser = "<div><a class='deleteUser' href='#' id='deleteUser-"+ i +"'onclick='com.impetus.ankush.configurationNew.deleteUser(this);'><input  style='width:22px;height:22px;' type='image' src='"+baseUrl+"/public/images/newUI-Icons/circle-minus.png' ></input></a>";
					var enabledColumn = '<div style=""><select id="endis-'+i+'" onclick="com.impetus.ankush.configurationNew.updateStatusField('+i+')"><option value="Enabled">Enabled</option><option value="Disabled">Disabled</option></select></div>';
					if(($('#userId').text() == users[i].username) || ((users[i].email == "user@company.com") && (users[i].firstName == "Admin"))){
						deleteUser = "";
		...
		loadEmailSection : function(result){
			if(Object.keys(result.output.email).length!=0){
    			...
    			$("#emailAccount").val(result.output.email.userName);
		...
		applyConfiguration:function(confUrl){
					...
					"email":
							{
	                                ...
	                                "userName":$("#emailAccount").val().trim(),
			...
			com.impetus.ankush.placeAjaxCall(url,"POST",false,data,
					function(result){
						...
						if(result.output.status == true && (document.getElementById('goToLoginOrDashboard') != null)){
							var url = baseUrl + '/app/state';
							var result = com.impetus.ankush.placeAjaxCall(url,"GET",false);
							if(result.output.state == "LOGIN"){
								$(location).attr('href',(baseUrl + '/auth/login'));
							}
							$('#errorBtn').hide();
							$('#error-div').hide();
						}else if(result.output.status == false){
							 	var errorCount = result.output.error.length;
								$('#errorBtn').text(com.impetus.ankush.configurationNew.getProperNounForError(errorCount)).show();
								for(var i=0;i<errorCount;i++){
									$("#popover-content").append('<div class="errorLineDiv" style="color:#404040">'+ result.output.error[i] + '</div>');
								}
								$('#error-div').show();
						}else{
							...
							com.impetus.ankush.dashboard.makeDashboardSelected();
						}
					},
				...
				if($(this).is(':visible')){
					...
					if((com.impetus.ankush.configurationNew.updateStatus(firstName)) || ($("#firstName-"+idIndex).text() == 'Empty') && ($("#firstName-"+idIndex).is(':visible'))){
						...
						if(firstNameMsgCount == 1){
							...
							com.impetus.ankush.configurationNew.mainFlag = false,
							firstNameMsg = "";
							errorMessage = com.impetus.ankush.configurationNew.errorCount + ". First Name is an alphanumeric input and cannot be blank";
							$("#popover-content").append('<div class="errorLineDiv"><a href="#userDatatable">'+errorMessage+'</a></div>');
						}
					}
					...
					if((com.impetus.ankush.configurationNew.updateStatus(lastName)) || ($("#lastName-"+idIndex).text() == 'Empty') && ($("#lastName-"+idIndex).is(':visible'))){
						...
						if(lastNameMsgCount == 1){
							com.impetus.ankush.configurationNew.errorCount++;
							com.impetus.ankush.configurationNew.mainFlag = false,
							errorMessage = com.impetus.ankush.configurationNew.errorCount + ". Last Name is an alphanumeric input and cannot be blank";
							$("#popover-content").append('<div class="errorLineDiv"><a href="#userDatatable">'+errorMessage+'</a></div>');
						}
					}
					...
					if((com.impetus.ankush.configurationNew.updateStatus(UserId)) || (($("#userId-"+idIndex).text() == 'Empty') && ($("#userId-"+idIndex).is(':visible')))){
						...
						if(userIdMsgCount == 1){
							com.impetus.ankush.configurationNew.errorCount++;
							com.impetus.ankush.configurationNew.mainFlag = false,
							errorMessage = com.impetus.ankush.configurationNew.errorCount + ". User ID is an alphanumeric non empty unique ID less than 50 and more than 4 character.";
							$("#popover-content").append('<div class="errorLineDiv"><a href="#userDatatable">'+errorMessage+'</a></div>');
						}
					}
					...
					if((com.impetus.ankush.configurationNew.updateStatus(email)) || ($("#emailId-"+idIndex).text() == 'Empty') && ($("#emailId-"+idIndex).is(':visible'))){
						...
						if(emailIdMsgCount == 1){
							com.impetus.ankush.configurationNew.errorCount++;
							com.impetus.ankush.configurationNew.mainFlag = false,
							errorMessage = com.impetus.ankush.configurationNew.errorCount + ". Email should have a proper format e.g. johndoe@company.com";
							$("#popover-content").append('<div class="errorLineDiv"><a href="#userDatatable">'+errorMessage+'</a></div>');
						}
					}
				}
		...
		$('#errorBtn').text(com.impetus.ankush.configurationNew.getProperNounForError(com.impetus.ankush.configurationNew.errorCount)).show();
		$('#error-div').show();
		},
		...
		actualValidate : function(key,fieldId){
			validationTypes ={
					...
					"confirmPassword":function(fieldId){return com.impetus.ankush.configurationNew.confirmPasswordValidation(fieldId);},
		...
		emptyValidation : function(fieldId){
			if(!com.impetus.ankush.validate.empty($("#"+fieldId).val())){
				...
				$("#popover-content").append('<div class="errorLineDiv"><a href="javascript:com.impetus.ankush.configurationNew.focusError(\''+ fieldId + '\');">'+com.impetus.ankush.configurationNew.errorCount + ". " + errorMsg+'</a></div>');
        ...
        licenseFileUpload : function() {
                var uploadUrl = baseUrl + '/uploadFile?category=license';
                ...
                $('#fileBrowse_JarFile').change(
                        function() {
                                ...
                                com.impetus.ankush.configurationNew.uploadFile(uploadUrl,"fileBrowse_JarFile");
                                com.impetus.ankush.configurationNew.removeFakePathFromPath('filePath_JarFile');
                        });
        },
        ...
        uploadFile : function(uploadUrl, fileId, callback, context) {
                ...
                $.ajaxFileUpload({
                        ...
                        success : function(result) {
                                ...
                                com.impetus.ankush.configurationNew.postLicense(fileUpload_JobJarPath);
                        },
                        error : function() {
